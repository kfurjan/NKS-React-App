...
const ItemsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const categoriesResponse = await axios.get("http://localhost:3000/Category", config);
        const subCategoriesResponse = await axios.get("http://localhost:3000/SubCategory", config);
        const productsResponse = await axios.get("http://localhost:3000/Product", config);
  ...
  const handleAddItem = async () => {
    ...
    const newItem = {
      ...
      totalPrice: products[selectedProductId].price * parseInt(selectedQuantity, 10),
    };

    try {
      const response = await axios.post("http://localhost:3000/Item", newItem, config);
      setItems((prevItems) => [...prevItems, response.data]);
      setNotificationMessage(`Item '${products[selectedProductId]?.name}' added successfully`);
  ...
  return (
    <Container>
            ...
            >
              <Row>
                ...
                <Col md={6}>
                  <Form.Group controlId="subcategorySelect">
                    ...
                    <Form.Control as="select" value={selectedSubCategoryId} onChange={handleSubCategoryChange}>
              ...
              <Row>
                <Col md={6}>
                  <Form.Group controlId="productSelect">
                    ...
                    <Form.Control as="select" value={selectedProductId} onChange={handleProductChange}>
                      {Object.values(products)
                        .filter((prod) => prod.subCategoryId.toString() === selectedSubCategoryId.toString())
                        .map((product) => (
                ...
                <Col md={6}>
                  <Form.Group controlId="quantitySelect">
                    ...
                    <Form.Control as="select" value={selectedQuantity} onChange={handleQuantityChange}>
                      {[...Array(10).keys()].map((x) => (
              ...
              <Row className="d-flex justify-content-between text-center">
                ...
                <Col xs={12} sm={6} md={3} className="text-end">
                  <p>
                    <strong>ProductNumber:</strong> {selectedProductDetails.productNumber}
                  </p>
                </Col>
              </Row>
      ...
      {showNotification && (
        <Alert variant="success" className={`notification-alert ${showNotification ? "show" : ""}`}>
          {notificationMessage}
        </Alert>
      )}
      <Row>
          ...
          <div className="d-flex justify-content-between">
            <div>
              {[...Array(totalPages).keys()].map((page) => (
                <Button key={page} variant={currentPage === page + 1 ? "primary" : "outline-primary"} onClick={() => setCurrentPage(page + 1)}>
                  {page + 1}
                </Button>
              ))}
            </div>
            <strong>
              Total of {items.reduce((sum, item) => sum + item.quantity, 0)} items: $
